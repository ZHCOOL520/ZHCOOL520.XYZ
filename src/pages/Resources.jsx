...
const downloads = [
  {
    icon: FiMonitor,
    title: 'Windows 系统镜像',
    subtitle: 'Win 11 · 10 · 8.1 · 7 · XP',
    desc: '从 Windows 11 到 Windows XP 全版本系统镜像合集，包含原版 ISO 和优化版本，适用于系统重装、虚拟机部署等场景。',
    ...
    links: [{ label: '百度网盘', url: 'https://pan.baidu.com/s/1xTAtytZd8OgLfQPhRvcoqA', note: '提取码：2333', code: '2333', type: '百度网盘' }],
  },
  {
    icon: FiPackage,
    title: '我的世界整合包',
    subtitle: '原版增强 · 科技模组 · 魔法冒险',
    desc: '精选 Minecraft 整合包资源合集，涵盖原版增强、科技模组、魔法冒险等多种玩法，一键安装即玩。',
    ...
    links: [{ label: '百度网盘', url: 'https://pan.baidu.com/s/1M-dmHBNFObMpkCnhRlfi0A?pwd=b2z2', note: '提取码：b2z2', code: 'b2z2', type: '百度网盘' }],
  },
  {
    icon: FiMusic,
    title: '音乐转码工具',
    subtitle: '酷狗 · QQ音乐 · 网易云',
    desc: '支持将加密音乐文件（酷狗/QQ音乐/网易云）转换为通用格式 MP3 / FLAC，方便跨设备播放与收藏。',
    ...
    links: [{ label: '蓝奏云', url: 'https://wwavs.lanzouv.com/b01d70zyuh', note: '密码：2333', code: '2333', type: '蓝奏云' }],
  },
];

function DownloadLink({ link, accent }) {
  const accentColor = accent.replace('text-', '').replace('-400', '');
  ...
  return (
    <a
      ...
      onClick={handleClick}
      className="group relative flex items-center justify-between px-6 py-5 rounded-2xl transition-all duration-300"
    >
      ...
      <div className={`absolute inset-0 rounded-2xl border border-transparent group-hover:border-${accentColor}-400/30 group-hover:shadow-[0_0_20px_rgba(0,240,255,0.08)] transition-all duration-300 pointer-events-none`} />
      ...
      <div className="relative flex items-center gap-5">
        ...
        <div>
          <div className={`font-semibold text-sm group-hover:${accent} transition-colors text-light-900 dark:text-white`}>
            {link.label}
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-[11px] font-mono ${copied ? 'text-neon-cyan' : 'text-light-500 dark:text-gray-500'} transition-colors`}>
              {copied ? `已复制 ${link.code} ✓` : link.note}
            </span>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-light-200/80 dark:bg-dark-600/80 text-light-500 dark:text-gray-500 font-mono border border-light-300/30 dark:border-white/5">{link.type}</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
        {copied ? (
          <span className="text-[11px] font-mono text-neon-cyan">已复制</span>
        ) : (
          <>
            <span className="text-[11px] font-mono text-neon-cyan">打开</span>
            <FiExternalLink className="text-neon-cyan" size={15} />
          </>
        )}
      </div>
    </a>
    ...
    <PageLayout maxWidth="max-w-3xl">
      ...
      >
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-3 mb-8 px-5 py-2.5 rounded-full border border-white/[0.08] dark:border-white/[0.06] bg-white/[0.03] dark:bg-white/[0.02] backdrop-blur-xl"
        >
        ...
        <p className="text-light-500 dark:text-gray-500 font-mono text-sm max-w-lg mx-auto leading-relaxed">
          {'// 精选工具与资源合集 · 持续更新中'}
        </p>
      </motion.div>
      ...
      <div className="space-y-8">
        {downloads.map((item, i) => (
          ...
          >
            {/* Card Container */}
            <div className={`group relative rounded-3xl overflow-hidden transition-all duration-500 ${item.glow}`}>
              ...
              <div className="relative rounded-3xl bg-light-100/80 dark:bg-dark-900/80 backdrop-blur-2xl p-7 sm:p-9">
                ...
                <div className="flex items-start gap-5 mb-6">
                  <motion.div
                    ...
                    className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${item.iconBg} flex items-center justify-center flex-shrink-0 shadow-lg ${item.glow}`}
                  >
                    <item.icon className={item.accent} size={28} />
                  </motion.div>
                  <div className="pt-1">
                    <h2 className="text-2xl font-bold text-light-900 dark:text-white mb-1.5">{item.title}</h2>
                    <p className={`text-xs font-mono ${item.accent} tracking-wide`}>{item.subtitle}</p>
                  </div>
                </div>

                {/* Description */}
                <p className="text-light-600 dark:text-gray-400 text-sm leading-relaxed mb-7 pl-[84px]">
                  {item.desc}
                </p>
                ...
                <nav aria-label={`${item.title} 下载链接`} className="space-y-2">
                  {item.links.map((link, j) => (
                    <DownloadLink key={j} link={link} accent={item.accent} />
                  ))}
                </nav>
              </div>
            </div>
      ...
      >
        <div className="inline-flex items-center gap-3 px-5 py-3 rounded-full border border-white/[0.04] bg-white/[0.01]">
          ...
          <p className="text-xs text-light-400 dark:text-gray-600 font-mono">
