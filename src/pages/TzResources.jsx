...
export default function TzResources() {
  ...
  useGSAP(() => {
    ...
    const badges = el.querySelectorAll('.tzr-badge');
    if (badges.length) {
      gsap.set(badges, { autoAlpha: 0, scale: 0.7, y: 40 });
      gsap.to(badges, { autoAlpha: 1, scale: 1, y: 0, duration: 0.55, stagger: 0.2, delay: 0.3, ease: 'elastic.out(1,0.5)' });
    }
    const catTitle = el.querySelector('.tzr-cat-title');
    if (catTitle) { gsap.set(catTitle, { autoAlpha: 0, y: 25 }); triggers.push(ScrollTrigger.create({ trigger: catTitle, start: 'top 92%', onEnter: () => gsap.to(catTitle, { autoAlpha: 1, y: 0, duration: 0.6, ease: 'expo.out' }), once: true })); }
    const cards = el.querySelectorAll('.tzr-card');
    if (cards.length) {
      gsap.set(cards, { autoAlpha: 0, scale: 0.8, rotationY: 15, y: 40 });
      triggers.push(ScrollTrigger.create({ trigger: cards[0], start: 'top 90%', onEnter: () => gsap.to(cards, { autoAlpha: 1, scale: 1, rotationY: 0, y: 0, duration: 0.55, stagger: 0.15, ease: 'back.out(1.5)' }), once: true }));
    }
    ...
    <div ref={pageRef} className="min-h-screen">
      ...
      <section className="py-12 px-6">
        <div className="max-w-xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-4">
          <div className="tzr-badge glass-card-sm flex items-center gap-3">
      ...
      <section className="py-16 sm:py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="tzr-cat-title text-center mb-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {categories.map((cat, i) => (
              <div key={i} className="tzr-card glass-card-sm group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500 flex items-center gap-1"><span className="inline-block w-1.5 h-1.5 rounded-full bg-indigo-500/60" />{cat.desc}</p>
                  </div>
                </div>
              </div>
            ))}
